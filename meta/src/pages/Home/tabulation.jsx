...
const Tabulation = () => {
  ...
  return (
    <div className="flex items-start max-md:flex-col">
      {/* sidebar */}
      <div className="flex max-w-[240px] w-full flex-col justify-start items-start max-md:flex-row max-md:items-stretch max-md:max-w-none">
        <button onClick={() => setActiveTab('founders')} className={clsx('relative p-7 w-full text-start duration-150 ease-in-out max-md:p-4 max-md:text-center', activeTab === 'founders' ? 'text-green-500' : '')}>
          <span className="relative z-10">Our founders</span>
          <div className={clsx('absolute right-0 top-0 h-full max-md:bottom-0 max-md:top-auto max-md:inset-x-0 duration-150 bg-neutral-700 opacity-75', activeTab === 'founders' ? 'w-full max-md:h-full' : 'w-0 max-md:h-0')}></div>
        </button>
        <button onClick={() => setActiveTab('partners')} className={clsx('relative p-7 w-full text-start duration-150 ease-in-out max-md:p-4 max-md:text-center', activeTab === 'partners' ? 'text-green-500' : '')}>
          <span className="relative z-10">Partners</span>
          <div className={clsx('absolute right-0 top-0 h-full max-md:bottom-0 max-md:top-auto max-md:inset-x-0 duration-150 bg-neutral-700 opacity-75', activeTab === 'partners' ? 'w-full max-md:h-full' : 'w-0 max-md:h-0')}></div>
        </button>
        <button onClick={() => setActiveTab('future')} className={clsx('relative p-7 w-full text-start duration-150 ease-in-out max-md:p-4 max-md:text-center', activeTab === 'future' ? 'text-green-500' : '')}>
          <span className="relative z-10">The future</span>
          <div className={clsx('absolute right-0 top-0 h-full max-md:bottom-0 max-md:top-auto max-md:inset-x-0 duration-150 bg-neutral-700 opacity-75', activeTab === 'future' ? 'w-full max-md:h-full' : 'w-0 max-md:h-0')}></div>
        </button>
      </div>
      {/* tab content */}
      <div className="flex-1 w-2/3 p-12 bg-neutral-700 bg-opacity-75 max-md:w-full">
        {activeTab === 'founders' && (
